...
// Quelldateien — siehe Grenzen unten).
// ---------------------------------------------------------------------------
...
const COLOR_NAMES = 'slate|teal|amber|green|rose|red|blue|sky|purple|violet|fuchsia|emerald'
...
function extractSegments(fileText: string): string[] {
...
interface Pair { bg: string; text: string; files: Set<string> }
...
describe('Dark-Mode-Kontrast (WCAG AA >= 4.5:1 für Text)', () => {
  const pairs = [...collectPairs().values()]
