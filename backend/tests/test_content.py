...
def test_module_meta_has_prereqs_and_order():
    with TestClient(app):
        ...
        try:
            ...
            assert any(m["key"] == "ports" and m["order"] == 7 for m in metas)
            assert any(m["key"] == "nat" and m["order"] == 8 for m in metas)
            assert any(m["key"] == "dns" and m["order"] == 9 for m in metas)
            assert any(m["key"] == "dhcp" and m["order"] == 10 for m in metas)
            ...
            assert any(m["key"] == "troubleshooting" and m["order"] == 16 for m in metas)
            assert any(m["key"] == "wireshark" and m["order"] == 17 for m in metas)
